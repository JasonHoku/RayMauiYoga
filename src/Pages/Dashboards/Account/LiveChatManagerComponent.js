...
class NoteManagerComponent extends Component {
  ...
  getData() {
    ...
    try {
      this.state.authVar = axios
        .get(`https://api.raymauiyoga.com/live-chats`, {
          headers: {
            "content-type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("jwt")}`,
          },
  ...
  purgeOldChats() {
    ...
    for (var i = 0; i < this.state.getOldTimestamp.length; i++) {
      ...
      if (new Date(this.state.getOldTimestamp[i]) < myStartDate) {
        axios
          .delete(
            `https://api.raymauiyoga.com/live-chats/${this.state.getDataEZID[i]}`,
  ...
  purgeByID = () => {
    axios
      .delete(
        `https://api.raymauiyoga.com/live-chats/${this.state.deleteIDVar}`,
