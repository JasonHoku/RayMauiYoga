...
export default function MainThreeJS() {
	...
	useEffect(() => {
		...
		roundInfo.current = { isRoundActive: false, roundNumber: 1, patternList: [], tilePatternDecided: false, clickData: { userPattern: [], userClickCount: 0, }, username: "Anonymous" }
...
void main() {
    ...
    gl_FragColor = ( texture2D( baseTexture, vUv ) + vec4( 1.0 ) * texture2D( bloomTexture, vUv ) );
		...
		renderer.setClearColor(0xfaffff, 1);
		...
		camera.position.set(20, 0, window.innerWidth <= 800 ? 32 : 33);

		camera.lookAt(10, 0, -50);

		renderer.setClearColor(0xeeffff, 0.1);
		...
		function setupScene() {
			...
			for (let i = 0; i < 56; i++) {
				...
				sceneLines.position.x = -25 + (1 * i) + (i * 0.3)
				sceneLines.position.y = -10
				sceneLines.position.z = -50;
				if (i % 3 === 1)
					sceneLines.rotation.z = Math.random() * -50;
		...
		function darkenNonBloomed(obj) {
			...
			if (obj.isMesh && bloomLayer.test(obj.layers) === false && obj.name !== "text") {
